...
pub struct LogEntry {
    pub append_file_index: u64, // index of the entry in the append only log
    pub log_index: u64,         // incremental counter of this entry in the log
    pub file_index: u64,        // the index in the pointers log
    // serialized size of the entry field
    pub ser_size: u64,
    // the actual entry
    pub entry: PrevEntry,
    // pointers to the previous and next entries in the log
    pub log_pointers: LogPointers,
    // pointers the the previous and next entries in the total order
    pub to_pointers: TotalOrderPointers,
}

/// Serialization structure of an entry in the pointer log
const SER_U64_SIZE: u64 = 8; // Serialized u64 size
const SER_TOTAL_SIZE: u64 = 4 * SER_U64_SIZE; // Total size of an entry in the pointer log
const APPEND_INDEX_SER: u64 = 0; // First the location of the entry in the append only log.
const LOG_INDEX_SER: u64 = APPEND_INDEX_SER + SER_U64_SIZE; // Next is incramental log counter.
const PREV_TO_SER: u64 = LOG_INDEX_SER + SER_U64_SIZE; // Next is the location of the previous total order entry in the log.
const NEXT_TO_SER: u64 = PREV_TO_SER + SER_U64_SIZE; // Next is the location of the next total order entry in the log.

impl Display for LogEntry {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "LogEntry => log_index: {}, append file idx: {}, {}, {}, entry: {:?}",
            self.log_index, self.append_file_index, self.log_pointers, self.to_pointers, self.entry
        )
    }
}
...
impl LogEntry {
    ...
    pub fn from_file<F: RWS, AppendF: RWS>(
        idx: u64,
        state: &LogState<F, AppendF>,
    ) -> Result<LogEntry> {
        let mut f = state.f.borrow_mut();
        f.seek_to(idx)?;
        // the first u64 contains the index of the entry in the append only log
        let (append_file_index, _) = f.read_u64()?;
        // incremental counter of this entry in the log
        let (log_index, _) = f.read_u64()?;
        // compute the location of the previous entry in the log
        let prev_log = match log_index.gt(&1) {
            // the first entry does not have a previous entry
            true => idx
                .checked_sub(SER_TOTAL_SIZE)
                .expect("missing bytes of previous entry"),
            false => 0, // there is no previous entry
        };
        ...
        // Now read the entry from the append only log
        let mut append_f = state.append_f.borrow_mut();
        append_f.seek_to(append_file_index)?;
        // read the serialized bytes
        let (entry_info, internal_entry): (_, PrevEntry) = append_f.read_log()?;

        // check the hash of the internal operation
        internal_entry.check_hash(f.serialize_option())?;
        // setup the actually entry
        let mut entry = LogEntry {
            append_file_index,
            log_index,
            file_index: idx,
            ser_size: entry_info.bytes_consumed,
            entry: internal_entry,
            log_pointers: LogPointers::default(),
            to_pointers: TotalOrderPointers::default(),
        };
    ...
    /// Write the pointers to the ordered elements to the log file.
    /// This is called when the entry is first received,
    /// so the next pointer in the total order is unknow, and will be written
    /// by the next element.
    pub fn write_pointers_initial<F: RWS, AppendF: RWS>(
        &mut self,
        ...
        // Be sure the previous pointers are set up correctly.
        if cfg!(debug_assertions) {
            ...
            let (nxt_to_file_idx_disk, _prev_to_file_idx) = match to_pointers.prev.as_ref() {
                Some(prev) => {
                    ...
                    // seek to the next total order entry
                    f.seek_to(prev_ref.file_index /*+ prev_ref.ser_size.unwrap()*/ + NEXT_TO_SER)?;
            ...
            let (prev_to_file_idx_disk, _nxt_to_file_idx) = match to_pointers.next.as_ref() {
                Some(nxt) => {
                    ...
                    f.seek_to(nxt_ref.file_index /*+ nxt_ref.ser_size.unwrap()*/ + PREV_TO_SER)?;
                    let val = f.read_u64()?.0;
                    f.seek_to_end();
        ...
        // First update the previous total order entries next total order pointer
        if self.log_index <= 1 {
            ...
            // now update the previous entries next total order pointer
            f.write_u64(self.file_index)?;
        }

        // Now add our entry at the end of the log
        f.seek_to_end();
        // sanity check
        if cfg!(debug_assertions) {
        ...
        // First write the location of the entry in the append only log.
        f.write_u64(self.append_file_index)?;
        // Next write the incramental log counter.
        f.write_u64(self.log_index)?;
        ...
        match to_pointers.next.as_ref() {
            Some(nxt) => {
                ...
                let nxt_prev_file =
                    nxt_file_idx /*+ nxt.ptr.borrow().ser_size.unwrap()*/ + PREV_TO_SER;
...
impl PrevEntry {
    ...
    pub fn get_hash(&self) -> Hash {
        match self {
            ...
            PrevEntry::None => panic!("entry is none"),
        }
    }
...
impl TotalOrderPointers {
                ...
                match entry.log_index {
                    ...
                    _ => {
                        ...
                        match entry.entry {
                            ...
                            PrevEntry::None => panic!("prev entry is none"),
                        }
                    }
                }
            }
    ...
    pub fn check_ptrs<F: RWS, AppendF: RWS>(&self, state: &LogState<F, AppendF>) {
    ...
    #[inline(always)]
    pub fn get_prev_to<F: RWS, AppendF: RWS>(
        &self,
        state: &LogState<F, AppendF>,
    ) -> Option<StrongPtr> {
    ...
    #[inline(always)]
    pub fn get_next_to<F: RWS, AppendF: RWS>(
        &self,
        state: &LogState<F, AppendF>,
    ) -> Option<StrongPtr> {
...
pub fn set_to_pointers_append_to_log<F: RWS, AppendF: RWS>(
    to_pointers: &StrongPointers,
    ...
    new_item
        .ptr
        .borrow_mut()
        .write_pointers_initial(&to_pointers, state)
        .unwrap();
}
...
impl LogEntryKeep {
    ...
    fn load_pointer<F: RWS, AppendF: RWS>(&mut self, state: &LogState<F, AppendF>) {
        self.ptr = Some(state.get_ptr(self.file_idx))
    }
...
impl LogEntryStrong {
    ...
    #[inline(always)]
    pub fn log_entry_keep<F: RWS, AppendF: RWS>(
        &mut self,
        state: &LogState<F, AppendF>,
    ) -> LogEntryKeep {
        LogEntryKeep {
    ...
    #[inline(always)]
    pub fn strong_ptr_idx<F: RWS, AppendF: RWS>(
        &mut self,
        state: &LogState<F, AppendF>,
    ) -> StrongPtrIdx {
        StrongPtrIdx {
    ...
    #[inline(always)]
    pub fn get_ptr<F: RWS, AppendF: RWS>(&mut self, state: &LogState<F, AppendF>) -> StrongPtr {
        if self.ptr.is_none() {
            self.ptr = Some(state.get_ptr(self.file_idx));
        }
...
impl LogEntryWeak {
    ...
    fn as_log_entry_strong<F: RWS, AppendF: RWS>(
        &self,
        state: &LogState<F, AppendF>,
    ) -> LogEntryStrong {
        LogEntryStrong {
    ...
    pub fn to_strong_ptr_idx<F: RWS, AppendF: RWS>(
        &self,
        state: &LogState<F, AppendF>,
    ) -> StrongPtrIdx {
        StrongPtrIdx {
    ...
    pub fn get_ptr<F: RWS, AppendF: RWS>(&self, state: &LogState<F, AppendF>) -> StrongPtr {
        self.ptr
            .upgrade()
            .unwrap_or_else(|| state.get_ptr(self.file_idx))
    }
...
impl LogPointers {
    ...
    pub fn get_prev<F: RWS, AppendF: RWS>(
        &mut self,
        state: &LogState<F, AppendF>,
    ) -> Option<StrongPtr> {
    ...
    pub fn get_next<F: RWS, AppendF: RWS>(
        &self,
        state: &LogState<F, AppendF>,
    ) -> Option<StrongPtr> {
...
struct WeakToStrongIterRef<'a, F: RWS, AppendF: RWS, J: Iterator<Item = &'a LogEntryWeak>> {
    iter: J,
    state: &'a LogState<F, AppendF>,
}

impl<'a, F: RWS, AppendF: RWS, J: Iterator<Item = &'a LogEntryWeak>> Iterator
    for WeakToStrongIterRef<'a, F, AppendF, J>
{
...
struct WeakToStrongIter<'a, F: RWS, AppendF: RWS, J: Iterator<Item = LogEntryWeak>> {
    iter: J,
    state: &'a LogState<F, AppendF>,
}

impl<'a, F: RWS, AppendF: RWS, J: Iterator<Item = LogEntryWeak>> Iterator
    for WeakToStrongIter<'a, F, AppendF, J>
{
    type Item = StrongPtrIdx;
...
impl OuterSp {
    ...
    pub fn get_ops_after_iter<'a, F: RWS, AppendF: RWS>(
        &'a self,
    ...
    fn get_early_ops<'a, I, J, F: RWS, AppendF: RWS>(
        &'a self,
    ...
    /// first_op is the first operation in the log in total order (this is used only if this is the first SP)
    pub fn check_sp_exact<F: RWS, AppendF: RWS>(
        &self,
        my_log_idx: LogIdx,
        new_sp: SpState,
        exact: &[EntryInfo],
    ...
    /// first_op is the first operation in the log in total order (this is used only if this is the first SP)
    fn get_last_op<F: RWS, AppendF: RWS>(
        &self,
        first_op: Option<LogEntryWeak>,
        state: &LogState<F, AppendF>,
    ) -> Result<StrongPtrIdx> {
    ...
    /// first_op is the first operation in the log in total order (this is used only if this is the first SP)
    fn get_iters<'a, F: RWS, AppendF: RWS>(
        &'a self,
        ...
        impl Iterator<Item = StrongPtrIdx> + 'a,
        TotalOrderAfterIter<'a, F, AppendF>,
    )> {
...
pub fn total_order_iterator<'a, F: RWS, AppendF: RWS>(
    start: &LogEntryStrong,
    forward: bool,
    state: &'a LogState<F, AppendF>,
) -> TotalOrderIterator<'a, F, AppendF> {
    TotalOrderIterator {
...
pub struct TotalOrderIterator<'a, F: RWS, AppendF: RWS> {
    pub prev_entry: Option<LogEntryStrong>,
    pub forward: bool,
    pub state: &'a LogState<F, AppendF>,
}

impl<'a, F: RWS, AppendF: RWS> Iterator for TotalOrderIterator<'a, F, AppendF> {
    type Item = StrongPtrIdx;
...
pub struct TotalOrderAfterIter<'a, F: RWS, AppendF: RWS> {
...
/// in a total order.
impl<'a, F: RWS, AppendF: RWS> Iterator for TotalOrderAfterIter<'a, F, AppendF> {
    type Item = StrongPtrIdx;
...
pub fn total_order_after_late_iter<'a, F: RWS, AppendF: RWS>(
    start: &StrongPtrIdx,
    // late_min: Option<EntryInfo>,
    state: &'a LogState<F, AppendF>,
) -> TotalOrderAfterIter<'a, F, AppendF> {
...
pub fn total_order_after_all_iter<'a, F: RWS, AppendF: RWS>(
    start: &StrongPtrIdx,
    state: &'a LogState<F, AppendF>,
) -> TotalOrderAfterIter<'a, F, AppendF> {
    TotalOrderAfterIter {
...
pub fn total_order_after_iter<'a, F: RWS, AppendF: RWS>(
    start: &StrongPtrIdx,
    state: &'a LogState<F, AppendF>,
) -> TotalOrderAfterIter<'a, F, AppendF> {
    TotalOrderAfterIter {
...
pub fn total_order_exact_iterator<'a, J, K, I, F: RWS, AppendF: RWS>(
    prev_not_included: I,
    log_iter: TotalOrderAfterIter<'a, F, AppendF>,
    exact: J,
    extra_info: K,
) -> TotalOrderExactIterator<'a, J, K>
where
...
pub fn total_order_check_iterator<'a, J, K, L, I, F: RWS, AppendF: RWS>(
    prev_not_included: I,
    log_iter: TotalOrderAfterIter<'a, F, AppendF>,
    included: J,
    not_included: L,
...
pub struct LogOpIterator<'a, F: RWS, AppendF: RWS>(LogIterator<'a, F, AppendF>);

impl<'a, F: RWS, AppendF: RWS> LogOpIterator<'a, F, AppendF> {
    /// Creates a new op iterator from a LogIterator.
    pub fn new(i: LogIterator<'a, F, AppendF>) -> Self {
        LogOpIterator(i)
    }
}

impl<'a, F: RWS, AppendF: RWS> Iterator for LogOpIterator<'a, F, AppendF> {
    type Item = StrongPtrIdx;
...
pub struct LogIterator<'a, F: RWS, AppendF: RWS> {
    prv_entry: Option<StrongPtrIdx>,
    state: &'a LogState<F, AppendF>,
    forward: bool,
}

impl<'a, F: RWS, AppendF: RWS> LogIterator<'a, F, AppendF> {
...
impl<'a, F: RWS, AppendF: RWS> Iterator for LogIterator<'a, F, AppendF> {
    type Item = StrongPtrIdx;
...
mod tests {
    ...
    fn make_log_entry(
        ...
        let file_index = match prev_log {
            None => 0,
            Some(prv) => prv.borrow().file_index + SER_TOTAL_SIZE,
        };
    ...
    fn make_outer_sp<F: RWS, AppendF: RWS>(
        id: Id,
        ti: &mut TimeTest,
        state: &LogState<F, AppendF>,
        ...
        let append_info = {
            ...
            let mut append_f = state.append_f.borrow_mut();
            append_f.append_log(&entry).unwrap()
        };
        let (log_entry, to_pointers, _log_pointers) = make_log_entry(
            append_info,
            entry,
    ...
    fn make_op<F: RWS, AppendF: RWS>(
        ...
        log_entry
            ...
            .write_pointers_initial(&to_pointers, state)
            .unwrap();
        debug!("entry size {}", log_entry.borrow().ser_size);
        state
            .f
            .borrow_mut()
            .seek_to(log_entry.borrow().file_index)
    ...
    fn serialize_entry() {
        let f = RefCell::new(
            open_log_file("log_files/entry_log0.log", true, false, FileSR::new).unwrap(),
        );
        let append_f = RefCell::new(
            open_log_file("log_files/entry_append_log0.log", true, true, FileSR::new).unwrap(),
        );
        let mut ti = TimeTest::new();
        let id = 0;
        let m = RefCell::new(HashItems::default());
        let state = LogState { m, f, append_f };
